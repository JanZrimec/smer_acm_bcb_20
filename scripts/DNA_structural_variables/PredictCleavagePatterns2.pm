...
sub discretizeOrchid2 {

    my $seq = shift;
    my @bins = qw(0.0924 0.3523 0.6225);
    my $pattern = &predictOrchid2($seq);
    my $discretized;
    
    
    
    while (scalar(@{$pattern})) {
        my $pat = shift @{$pattern};
        my $d = 0;
        for (my $i = 0; $i <= $#bins; $i++) {
            if (sprintf("%0.4f", $pat) <= $bins[$i]) {
                $discretized .= $i;
                $d = 1;
                last;
            }
...
sub predictOrchid2 {
	...
	my $sequence = Bio::Seq->new(-id => 'input', -seq => "$seq");

	my $rc_seq = $sequence->revcom->seq;


    my $patf = &predictSequenceCleavagePatternWithTetramers(uc $seq);
    my $patr = &predictSequenceCleavagePatternWithTetramers(uc $rc_seq);

    my @patr_rev = reverse(@{$patr});
    my $patr_r = \@patr_rev;
    ...
    for (my $i = 0; $i <= $#{$patf}; $i++) {
        if ($i >= 3) {
            push @averaged_pattern, (${$patf}[$i] + ${$patr_r}[$i - 3]) / 2;
        }
        else {
            push @averaged_pattern, ${$patf}[$i];
        }
    }
